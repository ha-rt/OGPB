...
class ModerationLogger():
    ...
    async def log_ban(self, ctx: ApplicationContext, member: User, reason, evidence, notes):
        ...
        embed.fields = [field for field in embed.fields 
                if not ((field.name == "Evidence" and evidence is None) or
                        (field.name == "Ban Note" and notes is None))]

        embed.thumbnail = member.display_avatar.url
        embed = load_data_into_embed(embed,
                                     {"user_id": member.id, "moderator_id": ctx.author.id, "reason": reason, "evidence": evidence, "note": notes, "timestamp_unix": str(int(time.time()))}
                                     )
    ...
    async def log_unban(self, ctx: ApplicationContext, member: User, notes):
        ...
        embed.fields = [field for field in embed.fields 
                if not (field.name == "Note" and notes is None)]

        embed.thumbnail = member.display_avatar.url
        embed = load_data_into_embed(embed,
                                     {"user_id": member.id, "moderator_id": ctx.author.id, "note": notes, "timestamp_unix": str(int(time.time()))}
                                     )
        await log_info_to_guild(ctx, "moderation", embed)